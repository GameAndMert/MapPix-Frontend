...
const DEFAULT_OG_IMAGE = 'https://localhost/mappix/superchunk/world/3/0/0';
...
async function getPosAndOgImage(canvases: Array<CanvasType>, pos: string): Promise<{ img: string, pos?: { x: number, y: number, zoom: number, canvas: string } }> {
  ...
  if (selectedZoom === 0) {
    ...
    return { pos: { x, y, zoom, canvas: canvas.id }, img: `https://localhost/mappix/chunk/${canvas.id}/${toLoadX}/${toLoadY}` };
  } else {
    ...
    return { pos: { x, y, zoom, canvas: canvas.id }, img: `https://localhost/mappix/superchunk/${canvas.id}/${selectedZoom}/${toLoadX}/${toLoadY}` };
  }
}
